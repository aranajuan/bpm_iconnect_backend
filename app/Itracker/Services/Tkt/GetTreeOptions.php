	...
	public static function GO($Context) {
		...
		if ( $topts ["object"] && $topts ["object"] instanceof \Itracker\Option ) {
			if ( $topts ["object"]->get_prop ( "itform" ) ) {
				...
				$rta_tree->addValue ( new ResponseElement ( 'opendata', new ResponseElement ( 'msj', 'No se necesitan datos adicionales. Puedes generar el itracker.' ) ) );
			}
			return $rta_tree;
		}
		/* Verificar si hay opciones */
		if ( !isset ( $topts ["options"] ) || count ( $topts ["options"] ) == 0 ) {
			throw new ItFunctionalException ( 'action/go/invalid', 'No hay destino valido', 'Sin opciones', $topts );
		}
